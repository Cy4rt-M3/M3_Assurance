...
const FRAMEWORK_ID_ALIASES: Record<string, string> = {
  ...
  'nist csf': 'nist_csf_2_0_2',
  'pci-dss': 'pci_dss_v4_0_1',
  gdpr: 'gdpr',
  dpdp: 'dpdp',
};
...
function ReportBuilderWizard({
  ...
  onGenerate: (report: Report) => void | Promise<void>;
  onComplete: () => void;
}) {
  ...
  function handleGenerate() {
    ...
    setGenerating(true);
    Promise.resolve(onGenerate(newReport))
...
export function ReportsSection() {
  ...
  const { data: backendReports = [] } = useM3Reports(engagementId);
  const reports = useReportsStore((state) => state.reports);
  ...
  useEffect(() => {
    if (backendReports.length > 0) {
      setReports(backendReports.map(m3ReportToDashboard));
    }
  }, [backendReports, setReports]);
  ...
  async function handleGenerateReport(report: Report) {
    const frameworkKey = report.framework.toLowerCase().replace(/\s+/g, ' ').trim();
    const frameworkIds = frameworkKey === 'all'
      ? []
      : FRAMEWORK_ID_ALIASES[frameworkKey]
        ? [FRAMEWORK_ID_ALIASES[frameworkKey]]
        : [];

    if (frameworkIds.length === 0) {
      toast({
        title: 'Framework not available',
        description: `"${report.framework}" is not mapped to an M3 engagement framework.`,
        variant: 'destructive',
      });
      throw new Error(`No framework mapping for "${report.framework}"`);
    }

    const created = await m3.generateReport(engagementId, frameworkIds, 'html');
    const dashboardReport = m3ReportToDashboard(created);
    addReport(dashboardReport);
    setSelectedReportId(created.report_id);
    toast({
      title: 'Report generated by the M3 pipeline',
      description: `${dashboardReport.title} (${created.report_id}) · composite score ${created.composite_score}%`,
    });
  }
