...
import { Shield, FileCheck, AlertTriangle, TrendingUp, Activity, FileText, Clock, CheckCircle2, XCircle, AlertCircle, Zap, ScanLine, Database, Target, FileBarChart, ChevronRight, ScrollText, Calendar, ChevronDown } from 'lucide-react';
import { complianceTrendData, riskDistributionData, riskMatrixData, notifications as notifData, tasks as allTasks } from '@/lib/assurance/mock-data';
...
import {
  ...
  useM3Engagements,
  useM3Reports,
} from '@/lib/assurance/backend';
import { useEngagementStore } from '@/hooks/use-engagement';
import {
  Select,
  SelectContent,
  SelectItem,
...
export function DashboardSection() {
  const { engagementId, setEngagementId } = useEngagementStore();
  const { data: engagements = [], isLoading: engagementsLoading } = useM3Engagements();
  const dashboard = useAssuranceDashboard(engagementId);
  const { data: storedReports = [] } = useM3Reports(engagementId);

  const loading = engagementsLoading || dashboard.loading;
  ...
  const validatedEvidence = dashboard.totalLinks;
  const pendingEvidence = 0;
  const expiredEvidence = 0;
  const criticalRisks = dashboard.criticalGaps;
  const openRisks = risks.filter((r) => r.status === 'open').length;
  const generatedReportCount = storedReports.length;
  const recentReports = storedReports.slice(0, 4).map((report) => ({
    id: report.report_id,
    title: `Compliance report (${report.framework_ids.length} framework${report.framework_ids.length === 1 ? '' : 's'})`,
    framework: report.framework_ids.join(', '),
    status: 'generated' as const,
    complianceScore: report.composite_score,
    generatedAt: '',
    scheduledAt: '',
  }));
  ...
  const variantFor = (value: number) =>
    value >= 80
      ? { color: 'text-success', bar: 'bg-success' }
      : value >= 60
        ? { color: 'text-warning', bar: 'bg-warning' }
        : { color: 'text-danger', bar: 'bg-danger' };

  const executiveMetrics: ScoreMetric[] = [
    {
      label: 'Compliance Score',
      value: avgCompliance,
      unit: '%',
      icon: Shield,
      ...variantFor(avgCompliance),
    },
    {
      ...
      unit: '%',
      icon: TrendingUp,
      ...variantFor(score.overall),
    },
    {
      label: 'Risk Score',
      value: Math.round(score.risk),
      unit: '%',
      icon: AlertTriangle,
      ...variantFor(score.risk),
    },
    {
      label: 'Evidence Coverage',
      value: Math.round(dashboard.completenessPct),
      unit: '%',
      icon: FileCheck,
      ...variantFor(dashboard.completenessPct),
    },
    {
      label: 'Control Coverage',
      value: Math.round(dashboard.coveragePct),
      unit: '%',
      icon: Target,
      ...variantFor(dashboard.coveragePct),
    },
    {
      label: 'Audit Readiness',
      value: Math.round((avgCompliance + dashboard.completenessPct) / 2),
      unit: '%',
      icon: CheckCircle2,
      ...variantFor((avgCompliance + dashboard.completenessPct) / 2),
    },
  ];
  ...
  const kpiStats = [
    ...
    },
    {
      label: 'Resilience Score',
      value: `${score.overall}%`,
      sub: `Compliance: ${score.compliance}%`,
      icon: TrendingUp,
      color: getScoreColor(score.overall),
      bg: getScoreColor(score.overall) === 'text-success' ? 'bg-success/10' : getScoreColor(score.overall) === 'text-warning' ? 'bg-warning/10' : 'bg-danger/10',
      borderColor: getScoreColor(score.overall) === 'text-success' ? 'border-success/20' : getScoreColor(score.overall) === 'text-warning' ? 'border-warning/20' : 'border-danger/20',
      trend: '+3%',
      trendUp: true,
    },
  ];
  ...
  const traceabilitySteps = [
    ...
    { label: 'Evidence', sub: 'Chain Validated', icon: Database, color: 'text-success', bg: 'bg-success/15', count: validatedEvidence },
    { label: 'Control', sub: 'Mapped', icon: Target, color: 'text-primary', bg: 'bg-primary/15', count: dashboard.controlsMapped },
    { label: 'Framework', sub: 'Evaluated', icon: Shield, color: 'text-purple-400', bg: 'bg-purple-500/15', count: dashboard.evaluatedFrameworkIds.length },
    { label: 'Report', sub: 'Generated', icon: FileBarChart, color: 'text-cyan-400', bg: 'bg-cyan-500/15', count: generatedReportCount },
  ];
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {dashboard.error && (
        <div className="flex items-center gap-2 rounded-xl border border-danger/30 bg-danger/10 px-4 py-3">
          ...
          <p className="text-xs text-danger">
            Live data unavailable: {String(dashboard.error.message)}. Verification data may be stale.
          </p>
        </div>
      )}
      ...
      <ExecutiveScorecard metrics={executiveMetrics} />
      ...
      <ComplianceMaturityMeter score={Math.round(score.overall)} />
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      {report.framework} · <span className="capitalize">{report.status}</span> · {report.generatedAt ? timeAgo(report.generatedAt) : 'latest'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0 ml-3">
...
function ComplianceMaturityMeter({ score: propScore }: { score?: number }) {
  const stages = [
  ...
  const currentScore = propScore ?? 73;
...
function ExecutiveScorecard({ metrics }: { metrics?: ScoreMetric[] }) {
  const fallbackMetrics: ScoreMetric[] = [
    { label: 'Compliance Score', value: 73, unit: '%', color: 'text-warning', bar: 'bg-warning', icon: Shield },
    { label: 'Resilience Score', value: 74, unit: '%', color: 'text-warning', bar: 'bg-warning', icon: TrendingUp },
    { label: 'Risk Score', value: 68, unit: '%', color: 'text-cyber', bar: 'bg-cyber', icon: AlertTriangle },
    { label: 'Evidence Coverage', value: 78, unit: '%', color: 'text-success', bar: 'bg-success', icon: FileCheck },
    { label: 'Control Coverage', value: 71, unit: '%', color: 'text-warning', bar: 'bg-warning', icon: Target },
    { label: 'Audit Readiness', value: 82, unit: '%', color: 'text-success', bar: 'bg-success', icon: CheckCircle2 },
  ];

  const scorecards = metrics ?? fallbackMetrics;

  const avgScore = Math.round(scorecards.reduce((acc, m) => acc + m.value, 0) / Math.max(1, scorecards.length));
...
function EngagementSelector({
  ...
  engagements: { engagement_id: string; name: string; organization: string }[];
  value: string;
  onChange: (id: string) => void;
}) {
  return (
    <Select value={engagements.some((e) => e.engagement_id === value) ? value : undefined} onValueChange={onChange}>
      <SelectTrigger className="w-[240px] h-9 text-xs bg-card border-border">
        <SelectValue placeholder="Select engagement" />
      </SelectTrigger>
      <SelectContent>
        {engagements.map((engagement) => (
          <SelectItem key={engagement.engagement_id} value={engagement.engagement_id}>
            {engagement.name} · {engagement.organization}
          </SelectItem>
        ))}
      </SelectContent>
    </Select>
