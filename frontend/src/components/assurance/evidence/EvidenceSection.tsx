...
function EvidenceDashboard({ items }: { items: Evidence[] }) {
...
function EvidenceRepository({
  ...
  }, [search, frameworkFilter, statusFilter, evidenceItems]);
...
export function EvidenceSection() {
  ...
  const liveEvidence: Evidence[] = dashboard.evidenceLinks.map((link) => {
    const ctrl = dashboard.evaluatedControls.find(
      (c) => c.control_id === link.control_id,
    );
    return {
      id: link.link_id,
      asset: ctrl?.name ?? link.control_id,
      source: link.verdict_id,
      framework:
        dashboard.frameworks.find((f) => f.id === ctrl?.framework_id)?.name ?? 'M3 Assurance',
      control: ctrl?.name ?? link.control_id,
      controlRef: link.control_id,
      evidenceType: 'configuration',
      validationStatus: 'validated',
      timestamp: '',
      confidenceScore: 1,
      owner: 'M3 Pipeline',
      immutableHash: link.evidence_hash,
      description: `Evidence chain link ${link.chain_position ?? 0} for control ${link.control_id}`,
      metadata: {
        verdict_id: link.verdict_id,
        chain_position: String(link.chain_position ?? 0),
      },
      linkedControls: [link.control_id],
      linkedRisks: [],
      linkedReports: [],
      auditTrail: [],
    };
  });
  const evidenceItems = dashboard.loading ? mockEvidenceItems : liveEvidence;
    ...
    <div className="p-6 space-y-4">
      ...
      <Tabs defaultValue="dashboard" className="w-full">
        ...
        <TabsContent value="dashboard">
          <EvidenceDashboard items={evidenceItems} />
        </TabsContent>
        <TabsContent value="repository">
          <EvidenceRepository items={evidenceItems} onSelect={setSelectedEvidence} />
        </TabsContent>
