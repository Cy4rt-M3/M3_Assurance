...
export function FrameworkComparisonDialog({
  open,
  onOpenChange,
  frameworks: providedFrameworks,
}: FrameworkComparisonDialogProps & { frameworks?: Framework[] }) {
  const frameworksList = providedFrameworks && providedFrameworks.length > 0 ? providedFrameworks : frameworks;
  // Default to first 3 frameworks so users see a comparison immediately.
  const [selectedIds, setSelectedIds] = useState<string[]>(
    frameworksList.slice(0, 3).map((fw) => fw.id),
  );

  const selectedFrameworks = useMemo<Framework[]>(() => {
    return selectedIds
      .map((id) => frameworksList.find((fw) => fw.id === id))
      .filter((fw): fw is Framework => Boolean(fw));
  }, [selectedIds, frameworksList]);
      ...
      >
        ...
        <div className="shrink-0 border-t border-border px-6 py-3 flex items-center justify-between gap-3 bg-card">
          <p className="text-[11px] text-muted-foreground">
            Comparing {selectionCount} of {frameworksList.length} available frameworks
          </p>
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
