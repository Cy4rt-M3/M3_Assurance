...
import type { RegistryControl } from '@/lib/assurance/backend';
import { useAssuranceDashboard } from '@/lib/assurance/backend';
...
function getFrameworkName(frameworkId: string): string {
  const fw = mockFrameworks.find((f) => f.id === frameworkId);
  return fw ? fw.name : frameworkId;
}

function toControl(
  ctrl: RegistryControl,
  status?: string,
  linkCount = 0,
): Control {
  ...
  return {
    ...
    owner: '',
    priority: 'medium',
    complianceStatus: met ? 'compliant' : status ? 'non_compliant' : 'not_assessed',
    evidenceCount: linkCount,
    riskMapping: ctrl.attack_mapping ?? [],
    lastValidation: '',
    mappedControls: [],
    requiredEvidence: 1,
    availableEvidence: linkCount > 0 ? 1 : 0,
    missingEvidence: linkCount > 0 ? 0 : 1,
    confidence: met ? 1 : 0,
  };
}
...
function FrameworkDashboard({ frameworks }: { frameworks: Framework[] }) {
...
function ControlMapping({
  ...
  const [selectedFramework, setSelectedFramework] = useState<string>(
    controls.some((c) => c.frameworkId === 'fw-iso27001') ? 'fw-iso27001' : controls[0]?.frameworkId ?? 'fw-iso27001',
  );
...
export function FrameworkSection() {
  const { engagementId } = useEngagementStore();
  ...
  const liveFrameworks = dashboard.loading ? mockFrameworks : dashboard.frameworks;
  const liveControls: Control[] = dashboard.loading
    ? mockControls
    : dashboard.evaluatedControls.map((ctrl) => {
        const linkCount = dashboard.evidenceLinks.filter(
          (link) => link.control_id === ctrl.control_id,
        ).length;
        return toControl(ctrl, dashboard.controlStatusMap[ctrl.control_id], linkCount);
      });
    ...
    <div className="p-6 space-y-5">
      ...
      <Tabs defaultValue="dashboard" className="w-full">
        ...
        <TabsContent value="dashboard">
          <FrameworkDashboard frameworks={liveFrameworks} />
        </TabsContent>

        <TabsContent value="controls">
          <ControlExplorer frameworks={liveFrameworks} controls={liveControls} onSelect={setSelectedControl} />
        </TabsContent>

        <TabsContent value="mapping">
          <ControlMapping frameworks={mockFrameworks} controls={mockControls} />
        </TabsContent>

        <TabsContent value="evidence">
          <EvidenceRequirements frameworks={mockFrameworks} controls={mockControls} />
        </TabsContent>
      </Tabs>
