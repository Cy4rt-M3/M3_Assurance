...
// function ControlMapping() {
  function ControlMapping({ frameworks, controls }: {
    frameworks: Framework[];
    controls: Control[];
  }) {
  const [selectedFramework, setSelectedFramework] = useState<string>('nist_csf_2_0');
...
// function EvidenceRequirements() {
  function EvidenceRequirements({ frameworks, controls }: {
    frameworks: Framework[];
    controls: Control[];
  }) {
  const [selectedFramework, setSelectedFramework] = useState<string>('all');
                  ...
                  >
                    ...
                    <TableCell>
                      ...
                      >
                        ...
                        {getFrameworkName(ctrl.frameworkId, frameworks)}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-xs text-center text-foreground">
  ...
  useEffect(() => {
    async function loadFrameworkData() {
      try {
        ...
        const mappedFrameworks: Framework[] = [];

        for (const fw of frameworkData) {
          const controlData = await getFrameworkControls(fw.framework_id);
          const mappedControls: Control[] = [];

for (const ctrl of controlData) {
  const mappings = await getControlMappings(ctrl.control_id);

  const mappedControlsData = mappings.map((mapping: any) => ({
    framework: mapping.target_control_id.split(':')[0],
    controlRef: mapping.target_control_id,
    title: mapping.target_control_id,
    confidence:
      mapping.equivalence_level === 'Full'
        ? 1
        : mapping.equivalence_level === 'Partial'
        ? 0.5
        : 0,
  }));
  ...
  mappedControls.push({
    ...
    availableEvidence: 0,
    missingEvidence: 0,
    mappedControls: mappedControlsData,
    riskMapping: ctrl.attack_mapping || [],
  });
}


          // const mappedControls: Control[] = controlData.map((ctrl: any) => ({
          //   id: ctrl.control_id,
          //   frameworkId: ctrl.framework_id,
          //   controlRef: ctrl.control_id,
          //   title: ctrl.name,
          ...
          mappedFrameworks.push({
            ...
            compliance: 0,
            controlsCovered: 0,
            controlsTotal: mappedControls.length,
            evidenceLinked: 0,
            riskLevel: 'medium',
  ...
  //       const mappedFrameworks: Framework[] = frameworkData.map((fw: any) => ({
  //         id: fw.framework_id,
  //         name: fw.name,
  //         version: fw.version,
  //         description: fw.description,
  ...
  <FrameworkDashboard frameworks={frameworks} />
      ...
      <TabsContent value="dashboard">
        ...
        <TabsContent value="controls">
          <ControlExplorer frameworks={frameworks}
            controls={controls}onSelect={setSelectedControl} />
        </TabsContent>

        <TabsContent value="mapping">
          <ControlMapping frameworks={frameworks}
            controls={controls}/>
        </TabsContent>

        <TabsContent value="evidence">
          <EvidenceRequirements frameworks={frameworks}
            controls={controls}/>
        </TabsContent>
      </Tabs>
